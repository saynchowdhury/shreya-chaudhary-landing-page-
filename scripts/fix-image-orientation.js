...
async function fixOrientations() {
  console.log(
    "🔄 Restoring and auto-orienting vertical images from original git tree (commit 59b4437)...",
  );
  ...
  for (const file of rotatedFiles) {
    ...
    try {
      ...
      console.log(
        `✅ Fixed ${file}: now vertical ${newMeta.width}x${newMeta.height} (${(processedBuf.length / 1024).toFixed(1)} KB)`,
      );
    } catch (err) {
